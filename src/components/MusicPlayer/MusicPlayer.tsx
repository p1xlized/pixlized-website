...
export default function MusicPlayer({ albums, tracks }: MusicPlayerProps) {
  ...
  const [nowPlaying, setNowPlaying] = useState({ title: 'STANDBY MODE', meta: '--:-- | -- BPM' });
  ...
  const playTrack = useCallback((track: MusicTrack) => {
    ...
    newAudio.play().then(() => {
      ...
      setNowPlaying({ title: track.title, meta: `${track.genre || 'UNKNOWN'} | ${track.duration || '--:--'}` });
      setTrackInfo(`${track.genre || 'UNKNOWN'} | ${track.bpm ? track.bpm + ' BPM' : '120 BPM'}`);
  ...
  // Load album
  const loadAlbum = useCallback((idx: number) => {
  ...
  }, [isPlaying, audio, animFrameId, togglePlay, prevTrack, nextTrack, toggleMirror]);
  ...
  return (
    <div className="box-border flex h-full w-full flex-col gap-1 p-1 sm:gap-2 sm:p-2 md:p-4">
      {/* Header */}
      <header className="flex items-center justify-between border border-primary/40 bg-card/60 p-1 sm:p-2">
        <a
          href="/"
          className="flex items-center gap-2 rounded border border-primary/40 px-2 py-1 text-[10px] font-bold text-primary transition-colors hover:bg-primary/20 sm:text-xs"
        >
          <span>{boxDrawing.lt}</span><span>BACK TO MAIN</span><span>{boxDrawing.rt}</span>
        </a>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col items-end font-mono text-[9px] text-primary/60">
            <div>SYS_STAT: <span className="animate-pulse text-chart-2">* ONLINE</span></div>
      ...
      <main className="flex flex-1 flex-col gap-1 overflow-hidden sm:gap-2 md:flex-row">
        {/* Album Archive */}
        <section className="flex w-full flex-col overflow-hidden border border-primary/30 bg-card/40 md:w-64 lg:w-72">
          <div className="flex justify-between border-b border-primary/30 bg-card/60 px-2 py-1 text-xs font-bold tracking-widest text-primary/80">
            <span>{boxDrawing.tl}{boxDrawing.h}{boxDrawing.h} [ ALBUM ARCHIVE ] {boxDrawing.h}{boxDrawing.h}{boxDrawing.tr}</span>
            <span>{boxDrawing.lt}{albumsWithCovers.length}{boxDrawing.rt}</span>
          </div>
          <div className="custom-scroll flex-1 space-y-0.5 overflow-y-auto p-1 sm:p-2">
            {albumsWithCovers.map((album, idx) => (
              <div
                ...
                className="album-item group cursor-pointer border border-primary/10 bg-card/30 p-1.5 sm:p-2 text-xs transition-all hover:border-primary hover:bg-primary/20"
              >
                <div className="flex items-center justify-between font-bold text-foreground group-hover:text-primary">
                  <span className="truncate">
                    {boxDrawing.v} [{idx < 9 ? '0' + (idx + 1) : idx + 1}] {album.title}
                  </span>
          ...
          <div className="flex justify-between border-t border-primary/20 bg-card/60 p-1 text-[9px] text-muted-foreground">
            <span>{boxDrawing.tl} TOTAL TRACKS: {tracks.length} {boxDrawing.rt}</span>
            <span>{boxDrawing.tl} MEM: OK {boxDrawing.rt}</span>
          </div>
        </section>

        {/* Track Directory */}
        <section className="flex flex-1 flex-col overflow-hidden border border-primary/30 bg-card/40">
          <div className="flex justify-between border-b border-primary/30 bg-card/60 px-2 py-1 text-xs font-bold tracking-widest text-primary/80">
            <span>{boxDrawing.tl}{boxDrawing.h}{boxDrawing.h} [ TRACK DIRECTORY ] {boxDrawing.h}{boxDrawing.h}{boxDrawing.tr}</span>
            <span>{boxDrawing.lt} {selectedAlbum ? selectedAlbum.title.toUpperCase() : 'NO ALBUM LOADED'} {boxDrawing.rt}</span>
          </div>
          <div
            className="custom-scroll flex-1 space-y-0.5 overflow-y-auto p-1 sm:p-2"
          >
            {!selectedAlbum ? (
              <div className="flex h-full flex-col items-center justify-center text-xs text-muted-foreground">
                <pre className="mb-2 text-[9px] leading-[9px] text-primary/60">  SELECT AN ALBUM TO POPULATE AUDIO TRACKS            </pre>
              </div>
            ) : (
              selectedAlbum.tracks.map((t, i) => (
                <div
                  ...
                  className="track-row p-1.5 sm:p-2 text-xs border border-border bg-card/30 hover:bg-primary/20 hover:border-primary cursor-pointer flex items-center justify-between transition-all"
                >
                  <div className="flex items-center gap-2 truncate">
                    <span className="text-muted-foreground font-mono">
                      {i + 1 < 10 ? '0' + (i + 1) : i + 1}.
                    </span>
                    <span className="text-foreground font-bold truncate">{t.title}</span>
                  </div>
                  <div className="flex items-center gap-2 sm:gap-3 text-[9px] text-muted-foreground">
          ...
          <div className="flex flex-col gap-1.5 border-t border-primary/40 bg-card/80 p-2 sm:p-3 shadow-[0_-3px_10px_rgba(0,0,0,0.3)]">
            <div className="flex items-end justify-between border-b border-primary/20 pb-1 text-xs">
              <div className="truncate pr-2">
                ...
                <span className="ml-1 font-bold text-foreground">{nowPlaying.title}</span>
              </div>
            ...
            <div className="flex items-center gap-1.5 text-xs">
              <span className="w-8 text-right font-mono text-primary/80 sm:w-10">{currentTime}</span>
              <div
              ...
              <span className="w-8 font-mono text-muted-foreground sm:w-10">{duration}</span>
            </div>
            <div className="flex items-center justify-center gap-2 pt-1 sm:gap-3">
              ...
              >
                {boxDrawing.lt}{boxDrawing.lt} PREV {boxDrawing.rt}{boxDrawing.rt}
              </button>
              <button
                onClick={togglePlay}
                disabled={!hasAudio}
              ...
              >
                {isPlaying ? `${boxDrawing.lt} PAUSE ${boxDrawing.rt}` : `${boxDrawing.lt} PLAY ${boxDrawing.rt}`}
              </button>
              <button
                onClick={nextTrack}
                disabled={!canNext}
                className="cyber-btn border border-primary/40 px-3 py-1 text-[10px] font-bold text-primary hover:bg-primary/20 disabled:opacity-30 sm:px-4 sm:text-xs"
              >
                {boxDrawing.lt}{boxDrawing.lt} NEXT {boxDrawing.rt}{boxDrawing.rt}
              </button>
            </div>
          </div>
        </section>

        {/* Spectrograph & Artwork */}
        <section className="flex w-full flex-col overflow-hidden border border-primary/30 bg-card/40 md:w-64 lg:w-80">
          <div className="flex justify-between border-b border-primary/30 bg-card/60 px-2 py-1 text-xs font-bold tracking-widest text-primary/80">
            <span>{boxDrawing.tl}{boxDrawing.h}{boxDrawing.h} [ SPECTROGRAPH ] {boxDrawing.h}{boxDrawing.h}{boxDrawing.tr}</span>
            <span>{boxDrawing.lt} {vizStatus} {boxDrawing.rt}</span>
          </div>
          ...
          <div className="flex justify-between border-t border-primary/20 bg-card/60 p-1.5 text-[9px] text-muted-foreground">
            ...
            <span>{boxDrawing.v} {trackInfo}</span>
      ...
      <footer className="flex items-center justify-between border border-primary/30 bg-card/60 px-2 py-1 text-[10px] text-primary/60">
        <div className="flex gap-3">
          <span><strong className="text-primary">[SPACE]</strong> PLAY/PAUSE</span>
          <span><strong className="text-primary">[LEFT/RIGHT]</strong> SKIP</span>
          <span><strong className="text-primary">[ESC]</strong> STOP</span>
          <span><strong className="text-primary">[M]</strong> MIRROR</span>
        </div>
        <div className="animate-pulse font-bold text-primary">
          {status}
        </div>
        <div className="text-muted-foreground">
          {boxDrawing.v} {tracks.length} tracks {boxDrawing.v} {albumsWithCovers.length} albums {boxDrawing.v}
        </div>
      </footer>
    </div>
  );
